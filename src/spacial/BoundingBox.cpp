...
BoundingBox::BoundingBox(const ActivityGrid & ag, Point & pt, int depth) :
	activityGrid(ag) {
	const ActivityGrid::Coordinates & coords = activityGrid.getBoundingCoordinates();
	int gridX = coords.get<0> ();
	int gridY = coords.get<1> ();
	int gridZ = coords.get<2> ();
	...
	if ((pt.getX() > gridX) || (pt.getY() > gridY) || (pt.getZ() > gridZ)) {
		...
		boxElements[FrontBottomRight]
				= activityGrid.getBoundingBoxElement(Point(round_x + corner_plus, round_y, round_z), pt);
		boxElements[BackBottomLeft] = activityGrid.getBoundingBoxElement(Point(round_x, round_y + corner_plus, round_z), pt);
		boxElements[BackBottomRight] = activityGrid.getBoundingBoxElement(
				Point(round_x + corner_plus, round_y + corner_plus, round_z), pt);
		boxElements[FrontTopLeft] = activityGrid.getBoundingBoxElement(Point(round_x, round_y, round_z + corner_plus), pt);
		boxElements[FrontTopRight] = activityGrid.getBoundingBoxElement(
				Point(round_x + corner_plus, round_y, round_z + corner_plus), pt);
		boxElements[BackTopLeft] = activityGrid.getBoundingBoxElement(Point(round_x, round_y + corner_plus, round_z + corner_plus),
				pt);
...
std::ostream& operator<<(std::ostream & os, const BoundingBox & obj) {
	os << "BoundingBox::operator <<: " << "TODO" << std::endl;
	// forall in boxElements
		 {
			 std::map<BoundingBox::BoxOrientation, boost::shared_ptr<BoundingBoxElement> >::const_iterator it_boxElements = obj.getBoxElements().begin();
			 const  std::map<BoundingBox::BoxOrientation, boost::shared_ptr<BoundingBoxElement> >::const_iterator it_boxElements_end =  obj.getBoxElements().end();
			 while ( it_boxElements != it_boxElements_end){
